...
<div style="margin-bottom: 20px; text-align: right; border: 1px solid #eee; padding: 10px;">
    <form method="GET" action="<?php echo BASE_URL; ?>index.php">
        <!-- Input hidden để giữ controller, action, id -->
    ...
    <div class="pagination" style="text-align: center; margin-top: 20px;">
        <?php if (isset($total_pages) && $total_pages > 1): ?>
            ...
            <?php if ($current_page > 1): ?>
                ...
                <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=brand&id=<?php echo $brand_id; ?>&page=<?php echo $current_page - 1; ?>&sort=<?php echo urlencode($sort ?? ''); ?>&price=<?php echo urlencode($price_range ?? ''); ?>">
                    &laquo; Trước
                </a>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <a href="<?php echo BASE_URL; ?>index.php?controller=product&action=brand&id=<?php echo $brand_id; ?>&page=<?php echo $i; ?>&sort=<?php echo urlencode($sort ?? ''); ?>&price=<?php echo urlencode($price_range ?? ''); ?>" 
