...
<nav>
    ...
    <ul id="main-navbar" class="main-navbar">
        ...
        <li class="nav-search-item" style="flex-grow: 1; min-width: 180px; margin: 0 15px;"> 
            <form method="GET" action="<?php echo BASE_URL; ?>index.php">
                <input type="hidden" name="controller" value="product">
                <input type="hidden" name="action" value="search">
                
                <input type="text" id="navbar-search-input" name="query" 
                        placeholder="Tìm kiếm sản phẩm..." autocomplete="off" style="width: 100%;">
            </form>
            ...
            <style>
                /* Giữ nguyên style kết quả tìm kiếm */
                .search-result-item {
        ...
        <li style="margin-left: 0;"<?php if ($current_controller == 'cart') echo ' class="active"'; ?>>
        ...
        <?php if (isset($_SESSION['user_id'])): ?>
            <li class="dropdown" style="padding-left: 5px;">
                <a href="<?php echo BASE_URL; ?>index.php?controller=account&action=index" 
                   style="display: flex; align-items: center; gap: 8px; padding-left: 10px;" 
                   class="<?php if ($current_controller == 'account') echo 'active'; ?>">
                    <img src="<?php echo BASE_URL . $nav_avatar_path; ?>" 
                          style="width: 30px; height: 30px; border-radius: 50%; object-fit: cover;">
                    <span class="user-greeting">Xin chào, 
                        <?php 
                        // Hiển thị vai trò nếu là admin, hoặc tên rút gọn
                        echo ($_SESSION['user_role'] === 'admin') ? 'Admin' : htmlspecialchars($_SESSION['user_full_name']);
                        ?>!
                    </span>
                </a>
                <ul class="dropdown-menu">
